...
| **BIAN API** | A canonical, SD-scoped business API that adheres to BIAN semantics using the BIAN generic operation verbs. The full set of BIAN v14 generic operations is: **initiate** (create a new SD instance), **retrieve** (return details), **update** (modify), **record** (capture information), **execute** (perform an activity), **evaluate** (assess or score), **control** (apply a control action), **register** (register a new entry), **notify** (send a state-change notification), **request** (make a formal request), **grant** (authorise access or permission), **provide** (supply information on demand), **feedback** (submit review or outcome), and **exchange** (two-way data exchange). Not every operation applies to every SD; the applicable operations are determined by the SD's Functional Pattern. FinX Glue exposes initiate, retrieve, update, record, execute, evaluate, control, register, and notify across its published Glue service APIs. BIAN APIs are the contract of record in FinX. Platform and tenant variability is expressed via governed extensions, not API shape changes. | *BIAN Glossary and Standards - FinX Context* |
...
| **Functional Pattern** | The structural archetype assigned to each BIAN Service Domain that determines its behavior and the set of applicable generic operations. BIAN v14 defines the following core patterns used in FinX: **Fulfill** — manages the complete product/service lifecycle for a customer (e.g., Current Account, Savings Account, Term Deposit); supports initiate, retrieve, update, execute, and record. **Administer** — manages reference or configuration data with long-term persistence (e.g., Party Reference Data Directory, Customer Agreement, Document Directory, Customer Product and Service Directory); supports initiate, retrieve, update, register, and record. **Process** — orchestrates a complex, multi-step business process to a conclusion (e.g., Know Your Customer); supports initiate, retrieve, update, record, and execute. **Monitor** — observes and reports on a business state or position (e.g., Position Keeping); supports retrieve, update, record, and notify. **Transact** — executes a discrete, atomic business action with immediate completion (e.g., Payment Order); supports initiate, retrieve, and record. Knowing the Functional Pattern of an SD tells engineers which BIAN operations to expect and helps partners predict API shape before consulting the OpenAPI spec. | *BIAN v14 Specification* |
| **BIAN Business Scenario** | A BIAN-defined end-to-end transaction pattern that sequences interactions across multiple Service Domains to accomplish a specific business goal. Business Scenarios serve as blueprints for FinX's Business Abstraction Layer. Key onboarding-relevant scenarios include: **Retail Customer Onboarding** (Party Reference Data Directory → Know Your Customer → Customer Agreement → Savings Account); **Account Opening** (Customer Product and Service Directory → Current Account or Savings Account → Position Keeping); **KYC Refresh** (Know Your Customer → Party Reference Data Directory update). FinX Business APIs (target state) expose these multi-SD sequences as single callable endpoints. | *BIAN v14 Specification* |
| **BIAN Landscape** | The complete taxonomy of approximately 325 BIAN Service Domains organized into 20 Business Areas that together cover the full operational scope of a bank. Business Areas include: Financial Institutions, Sales and Service, Operations and Execution, Risk and Compliance, Business Direction and Support, and others. FinX Glue implements a targeted subset of SDs relevant to onboarding, account management, and payments; the scope is extended per client engagement by adding BIAN-aligned Glue services for additional SDs. The Landscape is the primary reference for identifying which SD owns a given capability when scoping new integrations. | *BIAN v14 Specification* |
